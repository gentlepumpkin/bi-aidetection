...
public class RichTextBoxEx
{
    ...
    public RichTextBoxEx(RichTextBox RTF, bool AutoScroll)
    {
        ...
        {
            this.KnownColors.Add(clr.Name.ToLower(), clr);
        }
    ...
    public void RtfColorUpdate()
    {
        ...
        foreach (string c in colors)
        {
            ...
            this.RtfColors.Add(clr.Name.ToLower(), myclr);
        }
    }
                ...
                if (eb > -1)
                {
                    ...
                    foreach (string nm in clrs)
                    {
                        ...
                        else
                        {
                            ...
                            Color c = new Color();
                            if (this.KnownColors.TryGetValue(nm.ToLower(), out c))
                            {
                                Updated = true;
                                clr = c;
                    ...
                    if (Updated)
                    {
                        this.UIOp(this._RTF, () => this.AppendText(txt, clr, fs));
                    }
                    else //put it back the way it was
                    {
                        this.UIOp(this._RTF, () => this.AppendText("{" + part, Color.White, FontStyle.Regular));
                    }
                }
                else
                {
            ...
            if (Source.Trim().Length > 0)
            {
                ...
                foreach (HighlightWord item in Tags)
                {
                    if (!CFClr.ContainsKey(item.Clr.Name.ToLower()))
                    {
                        ...
                        RtfText.Append(";" + this.RtfColors[item.Clr.Name.ToLower()].RtfClr);
                        num = num + 1;
                    }
                }
                ...
                if (!KeyWordColor.IsEmpty)
                {
                    if (!CFClr.ContainsKey(KeyWordColor.Name.ToLower()))
                    {
                        ...
                        RtfText.Append(";" + this.RtfColors[KeyWordColor.Name.ToLower()].RtfClr);
                        num = num + 1;
                    }
                }
                if (!SymbolColor.IsEmpty)
                {
                    if (!CFClr.ContainsKey(SymbolColor.Name.ToLower()))
                    {
                        ...
                        RtfText.Append(";" + this.RtfColors[SymbolColor.Name.ToLower()].RtfClr);
                        num = num + 1;
                    }
                }
                ...
                if (!CFClr.ContainsKey(this._RTF.ForeColor.Name.ToLower()))
                {
                    ...
                    RtfText.Append(";" + this.RtfColors[this._RTF.ForeColor.Name.ToLower()].RtfClr);
                }
                ...
                foreach (KeyValuePair<int, FoundItem> itm in ScannerResult)
                {
                    ...
                    if (LastIndex < itm.Value.Start)
                    {
                        ...
                        Chunk = this.ParseRtf(Source.Substring(LastIndex, (itm.Value.Start - LastIndex)));
                    ...
                    Chunk = this.ParseRtf(Source.Substring(itm.Value.Start, itm.Value.Length));
                ...
                {
                    Chunk = this.ParseRtf(Source.Substring(LastIndex, (Source.Length - LastIndex)));
    ...
    public void AppendText(string text, Color color, FontStyle FS)
    {
        try
        {
            ...
            this.CurrentTextLength = this.CurrentTextLength + text.Length;
